...
abstract class Base extends CI_Controller
{
     ...
     * @return string
     */
    private function outputCssFile($file, $media='all')
    {
        if(preg_match('#^https?://#',$file) && preg_match('#\.css$#',$file))
    ...
    public function outputCssFiles()
    {
        ...
        {
            if(is_array($file))
                echo $this->outputCssFile($file['file'], $file['media']);
            else
                echo $this->outputCssFile($file);
        }
    }
     ...
     * @return string
     */
    private function outputJsFile($file)
    {
        if(preg_match('#^https?://#',$file) && preg_match('#\.js$#',$file))
     ...
     */
    public function outputJsFiles()
    {
        foreach ($this->_getJsFiles() as $file)
        {
            echo $this->outputJsFile($file);
        }
    }
