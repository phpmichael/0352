...
<tr>
	<td><?=$BC->_getFieldTitle("position")?>: <span class="red">*</span></td>
	<td><?=form_input("position",set_value('position',@$position))?></td>
</tr>
<tr>
	<td><?=$BC->_getFieldTitle("experience")?>:</td>
	<td><?=form_dropdown("experience",$experienceArr,set_value('experience',@$experience))?></td>
</tr>
<tr>
	<td><?=$BC->_getFieldTitle("salary")?>:</td>
	<td><?=form_input("salary",set_value('salary',@$salary),"style='width:50px;' alt='999999' ")?> <?=language('uah')?></td>
</tr>
<tr>
...
<?if(@$image):?>
<tr>
	<td><?=img('images/data/s/jobs/'.$image."?no_cache=".time())?></td>
	<td><?=anchor_base('jobs/deleteimage/'.$id,language('delete'))?></td>
</tr>
<?endif?>
<tr>
