...
<tr>
	...
	<td>
		<?=form_dropdown("type",array('text'=>'text','password'=>'password','select'=>'select','textarea'=>'textarea','radio'=>'radio','checkbox'=>'checkbox','submit'=>'submit','button'=>'button','captcha'=>'captcha','richtext'=>'richtext','file'=>'file','date'=>'date','time'=>'time','content'=>'content','display'=>'display','hidden'=>'hidden'),set_value('type',@$type));?>
...
<?//default value?>
<tr <?if(in_array(@$type,array('password','submit','button','captcha','file','content'))):?>style="display:none;"<?endif?>>
	<td><?=$BC->_getFieldTitle("default_value")?>:</td>
	<td><?=form_input("value",set_value('value',@$value));?></td>
</tr>
<?//validation?>
<tr <?if(in_array(@$type,array('submit','button','content','display'))):?>style="display:none;"<?endif?>>
	<td><?=$BC->_getFieldTitle("validation")?>:</td>
	<td><?=form_input("validation",set_value('validation',@$validation),"class='largeinput'");?></td>
</tr>
<?//show on list?>
<tr <?if(in_array(@$type,array('submit','button','content'))):?>style="display:none;"<?endif?>>
...
<?//hint?>
<tr <?if(in_array(@$type,array('submit','button','checkbox','radio','content','hidden'))):?>style="display:none;"<?endif?>>
	<td><?=$BC->_getFieldTitle("hint")?>:</td>
	<td><?=form_input("hint",set_value('hint',@$hint),"class='largeinput'");?></td>
</tr>
<?//height?>
<tr <?if(in_array(@$type,array('radio','checkbox','submit','button','content','hidden','display'))):?>style="display:none;"<?endif?>>
...
<?//label position?>
<tr <?if(in_array(@$type,array('submit','button','content','hidden'))):?>style="display:none;"<?endif?>>
...
<?//label width?>
<tr <?if(in_array(@$type,array('submit','button','content','hidden'))):?>style="display:none;"<?endif?>>
	<td><?=$BC->_getFieldTitle("label_width")?>:</td>
	<td><?=form_input("label_width",set_value('label_width',@$label_width),"style='width:40px;'");?> <?=form_dropdown("label_width_units",array('px'=>'px','%'=>'%'),set_value('label_width_units',@$label_width_units),"style='width:50px;'");?></td>
</tr>
<?//html class?>
<tr <?if(in_array(@$type,array('hidden'))):?>style="display:none;"<?endif?>>
