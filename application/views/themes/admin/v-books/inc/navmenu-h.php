...
<div id="menu-left">
  ...
  <ul id="navmenu-h">
    ...
    <?if(userAccessSomeOf(array('books','orders','shipping','currency','discounts','discount_coupons'),'view')):?>
    <li><a href="javascript:void(0)"><?=language('shop')?></a>
    
      <ul>
      
        <?if(userAccess('books','view')):?>
        <li><?=$BC->buildLeftMenuItem('books','books')?>
        
            <ul>
            
                <?if(userAccess('products_categories','view')):?>
                <li><?=$BC->buildLeftMenuItem('products_categories','categories')?></li>
                <?endif?>
