...
if (typeof jQuery != 'undefined') {
	...
	$j(document).ready(function(){
		...
		$j('#toggle-ajax').css({display: 'inline'});
		
		// bind ajax event
		$j('#debug-ajax').bind("ajaxComplete", function(event, xmlrequest, ajaxOptions){ 
			
			// add a new row to ajax table
			$j('#debug-ajax table').append(
				'<tr class="even">' +
					'<td>' + $j('#debug-ajax table tr').size() +'<\/td>' + 
			...
			// stripe table
			$j('#debug-ajax table tbody tr:nth-child(even)').attr('class', 'odd');	
			
			// update count in toolbar
			$j('#toggle-ajax span').text($j('#debug-ajax table tr').size()-1);
