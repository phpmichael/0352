...
var parseCSS=function(sheet){
	var ret=[]
	jQuery(sheet=sheet.replace(/\/\*.*\*\//g,'').replace(/\s+/g,' ').split('}')).each(function(){
		var tmp={}
...
var addBG=function(root,bg,hover){
	jQuery(root).each(function(){
		var className=!!hover?'__mbgh__':'__mbg__'
...
var prcssCSS3selectors=function(sheet){
	jQuery(findSelector(parseCSS(sheet),/(:nth)|(:last)|(:first)|(:only)|(:not)|(:empty)/)).each(function(){
		jQuery(this.sel).css(prprCSSjson(this.styles))
	})
}

var prcssMBG=function(sheet){
	jQuery(findStyle(parseCSS(sheet),/background\s*:.+,.+/)).each(function(){
		var tmp=this,hover=false
		...
		jQuery(this.style.replace(/background\s*:\s*/,'').split(',').reverse()).each(function(){
			var t
		...
		if(+jQuery.browser.version>6&&hover)jQuery(function(){
			jQuery(document.styleSheets).last().each(function(){
				if(this.addRule){
					this.addRule('.__hover__ .__mbgh__','visibility:hidden;')
...
var rfrshMBG=function(){
	jQuery(mbgList).each(function(){
		this.find('.__mbg__').remove()
	})
	jQuery(rfrshList).each(function(){
		addBG(this.sel,this.bg,this.hover)
	})
}
	
jQuery('link[rel=stylesheet],style').each(function(){
	var el=this
	if(this.tagName.toLowerCase()=='style')
		prcssCSS3selectors(el.innerHTML)
		prcssMBG(el.innerHTML)
	if(this.tagName.toLowerCase()=='link')
		jQuery.get(this.href,{},function(ret){
