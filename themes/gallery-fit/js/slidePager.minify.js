...
var rollTo=function(n,clbck){if(!block){block=true
...
var findIdx=function(str){str=str.split('#')[1]
var idx=0,tmp
keeper.find('.page').each(function(){if(this.id==str){tmp=idx}
else{idx++}})
return tmp}
jQuery(opt.next).click(function(){if(curr+1<len)rollTo(curr+1)
else rollTo(0)
return false})
jQuery(opt.prev).click(function(){if(curr>0)rollTo(curr-1)
else rollTo(len-1)
return false})
jQuery(document).delegate(opt.links,'click',function(){rollTo(findIdx(this.href))
return false})
jQuery(window).resize(function(){rollTo(curr)})}})
