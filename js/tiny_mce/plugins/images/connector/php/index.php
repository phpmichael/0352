...
class TinyImageManager {
	...
	function TinyImageManager() {
		...
		$this->ALLOWED_FILES = array('doc', 'docx', 'ppt', 'pptx', 'xls', 'xlsx', 'pub', 'swf', 'zip', 'rar', 'rtf', 'pdf', 'psd', 'mp3', 'mp4', 'wav', 'wma', 'avi', 'flv', 'mov', 'mpeg', 'mpg', 'txt');
			...
			while (false !== ($file = readdir($handle))) {
				if ($file != "." && $file != "..") {
					...
					if(is_file($dir.'/'.$file) ) {
						...
						} elseif(in_array(strtolower($file_info['extension']),$this->ALLOWED_IMAGES)) {
							$files[$file]['imageinfo'] = getimagesize($dir.'/'.$file);
							$files[$file]['general'] = array(
								'filename' => $file,
								'name'	=> basename(strtolower($file_info['basename']), '.'.$file_info['extension']),
						...
						} elseif(in_array(strtolower($file_info['extension']),$this->ALLOWED_FILES)) {
							//$files[$file]['imageinfo'] = getimagesize($dir.'/'.$file);
							$files[$file]['general'] = array(
								'filename' => $file,
								'name'	=> basename(strtolower($file_info['basename']), '.'.$file_info['extension']),
	...
	function UploadFile($dir, $type) {
		...
		if(isset($_FILES['Filedata']['tmp_name'])) {
			...
			if (strpos($_FILES['Filedata']['name'], '.') !== false) {
				...
				$filename = substr($_FILES['Filedata']['name'], 0, strlen($_FILES['Filedata']['name']) - strlen($extension) - 1);
			} else {
			...
			/*$file = $md5.'.'.$extension;*/
			$filename = $this->GenerateFilename($dir,$filename,$extension);
			$file = $filename.'.'.$extension;
			...
			//if($pathtype == 'images') {
			if(in_array(strtolower($extension),$this->ALLOWED_IMAGES)) {
	...
	function GenerateFilename($dir,$filename,$extension)
	{
	...
	function ShowDir($dir, $type) {
		...
		$ret = '';
		foreach ($dir as $v) {
			if(in_array(strtolower($v['general']['ext']),$this->ALLOWED_IMAGES))
			{
			...
			{
				$file_type_icon = strtoupper($v['general']['ext']);
				if(strlen($file_type_icon)==4 && substr($file_type_icon,3)=='X') $file_type_icon = substr($file_type_icon,0,3);//for docx, xlsx, pptx
				
				$block = '
				<div class="imageImage">
					<img src="'.DIR_FILETYPES_ICONS.$file_type_icon.'.png" width="100" height="100" alt="'.strtoupper($v['general']['ext']).'" />
				</div>';
   ...
   <table class="imageBlock0" cellpadding="0" cellspacing="0" filename="'.$v['general']['filename'].'" fname="'.$v['general']['name'].'" ext="'.strtoupper($v['general']['ext']).'" path="'.$v['general']['path'].'" linkto="'.$v['general']['link'].'" fsize="'.$v['general']['size'].'" date="'.date('d.m.Y H:i',$v['general']['date']).'" fwidth="'.@$v['general']['width'].'" fheight="'.@$v['general']['height'].'" md5="'.$v['general']['md5'].'" '.@$middle_thumb_attr.' ftype='.$ftype.'><tr><td valign="bottom" align="center">
	...
	function GetThumb($dir, $md5, $filename, $mode, $width=100, $height=100) {
		$path = str_replace('\\','/',realpath(DIR_ROOT.'/'.$dir));
